...
- **Animated Weather Maps** — Canvas-based maps with rainfall particles, heat shimmer effects, and district labels
- **Historical Data Explorer** — Time series charts for any variable at any grid point
- **Model Validation** — RMSE, MAE, bias, R² metrics with predicted-vs-observed comparison
...
│   │   │   ├── WeatherMap.tsx      # Canvas animated map
...
Without real IMD data, `load_input_sequence()` reads from `data/processed/andhra_pradesh_sample.json` (365 days of synthetic data generated by `scripts/generate_sample_data.py`).
